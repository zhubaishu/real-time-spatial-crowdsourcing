...
void findRoute(vector<int>& route, unordered_map<int,bool>& contain, unordered_map<int,int>& lowB, vector<int>& tmp, int T) {
	...
	if ( T + lowB[tmp.back()] >= minT ) return;
	for ( int i = 0; i < route.size(); i++ ) {
		...
		if ( !contain[route[i]] ) {
			...
			if ( TT <= eT ) {
				...
				findRoute(route,contain,lowB,tmp,TT);
...
vector<int> best_route(int tid, int wid) {
	...
	unordered_map<int,int> lowB;
	for ( int i = 0; i < route.size(); i++ )
		contain[route[i]] = false;
	for ( int i = 0; i < route.size(); i++ ) {
		int x = tasks[route[i]].loc.x;
		int y = tasks[route[i]].loc.y;
		int minDis = abs(x-init_x) + abs(y-init_y);
		for ( int j = 0; j < route.size(); j++ )
			if ( i != j ) {
				int xx = tasks[route[j]].loc.x;
				int yy = tasks[route[j]].loc.y;
	...
	for ( int i = 0; i < route.size(); i++ ) {
		...
		if ( T <= eT ) {
			...
			tmp.push_back(route[i]);
			contain[route[i]] = true;
			findRoute(route,contain,lowB,tmp,T);
			contain[route[i]] = false;
		}
	}
	route.pop_back();
...
void bnb_assign(vector<int>& candidate, int tid) {
	int min = INT_MAX;
