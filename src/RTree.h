...
public:
  ...
  public:
    ...
    bool IsNotNull()                              { /*std::cout << "IsNotNull m_tos = " << m_tos << std::endl;*/ return (m_tos > 0); }
...
RTREE_QUAL::RTree()
{
  ...
  //ASSERT(sizeof(DATATYPE) == sizeof(void*) || sizeof(DATATYPE) == sizeof(int)); // ÓÉÓÚ×Ô¶¨ÒåµÄÀàsizeof²»·ûºÏÕâ¸öASSERT£¬µ«ÊÇ´«Ö¸Õë»¹»áÓÐbug£¬ËùÒÔÈ¥µôÕâ¾äÅÐ¶Ï£¬¶ÔÊµ¼ÊÔËÐÐ²»»áÔì³ÉÓ°Ïì
...
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId)
{
  ...
  //std::cout << "a_dataId = " << a_dataId << std::endl;
  branch.m_child = NULL;
